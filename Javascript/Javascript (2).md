...
##  JavaScript Runtime
![image](https://miro.medium.com/max/1100/1*-QcXDhD9TnN7CcnYAV8Lxw.png)
## Closure, code and uses
### Most Basic
```
...
const actual = add10(5); // 15
```
## Callback vs Promise vs Async/Await
## Currying
## Object forEach, ForIn etc
## Array methods
### reduce
```
const users = [
...
## Redux Flow
## Redux MiddleWare
## Node process Immediate vs Next
## BOM vs DOM (Table form)
...
// console.log() has no explicit _return value
```
```
...
// 1. put the JavaScript **<script>** tags after <body> tag

// 2. Use an event listener to listen to eg. window's onload event, adding your code to that event listener will delay running your code until after the whole content on your page has been loaded.

// 3. A third way to make sure all your DOM has been loaded, is to wrap the DOM manipulation code with a timeout function of 0 ms. This way, this JavaScript code is re-queued at the end of the execution queue, which gives the browser a chance to finish doing some non-JavaScript things that have been waiting to finish before attending to this new piece of JavaScript.
```
```
<body>
...
alert(message);
// alert will pause further javascript execution until the prompt is answered.
// The use of alerts is usually discouraged
```
```
prompt(text, [ default ]);
```
```
var age = prompt("How old are you?"); // Input box, OK and Cancel buttons
...
```
```
// Message label, OK and Cancel buttons
...
```
### window.isNaN()
This function first checks if the value is a number, if not tries to convert it (*), and then checks if the resulting value is **NaN**.
```
isNaN( NaN ); // true
...
```
This last one is a bit tricky: checking if an Array is **NaN**. To do this, the Number() constructor first converts the array to a string, then to a number; this is the reason why isNaN([]) and isNaN([34]) both return **false** , but isNaN([1, 2]) and isNaN([ **true** ]) both return **true** : because they get converted to "", "34", "1,2" and "true" respectively. In general, **an array is considered NaN by isNaN() unless it only holds one element whose string representation**

**can be converted to a valid number**.

### Number.isNaN()
Number.isNaN() avoids the problem of window.isNaN() of forcefully converting the parameter to a number. Number.isNaN().
...
typeof null === 'object'
```
Some processes, such as JSON serialization, may strip **undefined** properties from objects. In contrast, **null** properties indicate will be preserved so you can explicitly convey the concept of an "empty" property.
```
let foo;
...
1 / 0 ; // Infinity
```
**Infinity** is a property of the global object (therefore a global variable) that represents mathematical infinity. It is a reference to Number.POSITIVE_INFINITY or Number.NEGATIVE_INFINITY.
```
...
_multiple_ lines, so the lines following them (e.g., self.postMessage(...) will not be ignored when run as JavaScript,

at least until they reach a _JavaScript_ comment, marked by /* and */. Such JavaScript comments are used in the

above example to ignore the remaining _HTML_ text (until the --> which is also ignored as JavaScript).
...
be directly opened in the browser ( F12 key in most browsers – see _Remarks_ below for further information) and
...
console.assert does _not_ throw an AssertionError (except in Node.js), meaning that this method is incompatible
...
Object. **prototype** .toString.call( _/foo/_ );
...
Template Literals ( _backticks_ ) `hello`.
...
Strings can be created using template literals ( _backticks_ ) `hello`.
...
If your string is enclosed (i.e.) in single quotes you need to escape the inner literal quote with _backslash_ \
...
```
Quotes in HTML strings can also be represented using &apos; (or &# 39 ;) as a single quote and &quot; ( or &# 34 ;) as
...
var hello = '<p class="special">I&apos;d like to say "Hi"</p>'; // valid String
```
_Note:_ The use of &apos; and &quot; will not overwrite double quotes that browsers can automatically place on
...
If a string has ' and " you may want to consider using template literals ( _also known as template strings in previous ES6_

_editions_ ), which do not require you to escape ' and ". These use backticks (`) instead of single or double quotes.
...
To detect whether a parameter is a _primitive_ string, use **typeof** :
...
```
A more robust solution is to not _detect_ a string at all, rather only check for what functionality is required. For
...
This is the case of character codes beyond 216 - 1 or 63553. These extended character codes or _code point_ values
...
JavaScript has native conversion from _Number_ to its _String representation_ for any base from _2 to 36_.

The most common representation after _decimal (base 10)_ is _hexadecimal (base 16)_ , but the contents of this section

work for all bases in the range.

In order to convert a _Number_ from decimal (base 10) to its hexadecimal (base 16) _String representation_ the _toString_

method can be used with _radix 16_.
...
```
If the number represented is an integer, the inverse operation for this can be done with parseInt and the _radix 16_
...
```
To convert an arbitrary number (i.e. non-integer) from its _String representation_ into a _Number_ , the operation must be
...
of _Numbers_ of the environment the conversions are happening in.
...
```
If you included that code and executed **new** DateObject() on January 20ʰ, 2019, it would produce an object with
...
_Note that_ Date() _called without arguments is equivalent to_ **new** Date(Date.now())_._
...
// Sample output: 0
```
Please note that 0 = January. This is because months range from _0_ to _11_ , so it is often desirable to add +1 to the
...
test("", "0"); // false;
```
_Side-note_ : Number("0") and **new** Number("0") isn't the same thing! While the former performs a type conversion,
...
**NaN** (" **N** ot **a N** umber") is a special value defined by the _IEEE Standard for Floating-Point Arithmetic_ , which is used when
...
Operands of the abstract equality operator are compared _after_ being converted to a common type. How this
...
Logical OR (||), reading left to right, will evaluate to the first _truthy_ value. If no _truthy_ value is found, the last value is
...
var g = '' || 'yay' || 'boo'; // g = 'yay'
```
Logical AND (&&), reading left to right, will evaluate to the first _falsy_ value. If no _falsey_ value is found, the last value is
...
comparison.So **null** is particular case because Number( **null** ); _//0_
...
```
To turn on a bit use bitwise _or_ | and the value corresponding to the bit. So if you wish to set the 2nd bit bitField

|= 0b10 will turn it on. If you wish to turn a bit off use bitwise _and_ & with a value that has all by the required bit on.
...
Ternary operators _always_ expect else conditions, otherwise you'll get a syntax error. As a workaround you could
...
An if condition can be expanded with an **else** block. The condition is checked _once_ as above, and if it evaluates to
...
As another example of using the _initial value_ parameter, consider the task of calling a function on an array of items,
...
The entire array
```
Additionally, map() provides an _optional_ second parameter in order to set the value of **this** in the mapping
...
If your array or array-like object is _numeric_ , that is, if all its elements are numbers, then you can use Math.min.apply
...
To generalize the reduce version we'd have to pass in an _initial value_ to cover the empty list case:
...
_Portions of this content from Array._ **_prototype_** _.entries by Mozilla Contributors licensed under CC-by-SA 2.5_
...
```
Be aware that the sort method operates on the array _in place_. It changes the array. Most other array methods
...
**_Characteristics of properties :_**

Properties that can be retrieved from an _object_ could have the following characteristics,
...
own
```
While creating the properties using _Object.defineProperty(ies)_ , we could set its characteristics except _"own"_.

Properties which are available in the direct level not in the _prototype_ level (__proto__) of an object are called as _own_
...
**_Methods of retrieving properties :_**
...
1. _data property_ : the property's name is associated with a value.
2. _accessor property_ : the property's name is associated with one or two accessor functions.
...
```
```
Math.PI Pi: the ratio of circle circumference to
diameter (π)
3.14
```
```
...
provide incorrect results if used as logical operators. The ^ operator is **not** the power operator ( _ab_ ).
...
```
Notice how the _prefix_ variant is used. This ensures that a temporarily variable isn't needlessly created (to return the
...
Graph of sine and cosine function: _(courtesy Wikipedia)_
...
1 0 1 0 => - 8 + 0 + 2 + 0 => - 6
```
_Note:_ Adding more 1 's to the left of a binary number does not change its value in two's compliment. The value 1010
...
The bitwise NOT operation ~a _flips_ the bits of the given value a. This means all the 1 's will become^0 's and all the^0 's
...
The bitwise XOR ( _exclusive or_ ) operation a ^ b places a 1 only if the two bits are different. Exclusive or means _either_
...
```
**Right Shift (** **_Sign-propagating_** **)**
...
- 5 >> 3 => - 1
_// -5: 1..111011
// -2: 1..111111 <= added three 1's from the left and chopped off 011 from the right_

**Right Shift (** **_Zero fill_** **)**
...
Declaring a variable **const** only prevents its value from being _replaced_ by a new value. **const** does not put any
...
for...of loops _do not_ work directly on plain Objects; but, it is possible to iterate over an object’s properties by
...
When you define a function, it creates a _scope_.
...
Invoked Function Expression (IIFE for short). These are essentially _self-executing anonymous functions_. They have
...
console.log(obj.a); // 2
```
However, changes made to (nested) properties _of_ such arguments, will be visible to the caller:
...
console.log(obj.a); // 5
```
This can be seen as a _call by reference_ : although a function cannot change the caller's object by assigning a new

value to it, it could _mutate_ the caller's object.
...
console.log([ 1 , 2 , 3 , 4 ].map(x => x * 2 )); _// [2, 4, 6, 8]_
...
Human.call( **this** ); _// inheriting from the base class_
**this** .name = name;
};

Customer. **prototype** = Object.create(Human. **prototype** ); _// creating the object_
...
bill.greet();
_// Hi, I am Bob, the Teacher_

carter.greet();
_// Hi, I am Carter_

andy.greet();
_// Hi, I am Andy, the Bill_
...
Static methods and properties are defined on _the class/constructor itself_ , not on instance objects. These are specified
...
```
However, they _are_ defined on subclasses:
...
that continues to _live_ in its enclosing scope that, at construction time, has been preserved.

Making use of this pattern - emulating private members via privileged public methods - one should keep in mind

that, with every instance, additional memory will be consumed for every _own property_ method (for it is code that
...
console.log(person.firstname, person.lastname); _// John Doe_

person.firstname = 'Foo';
person.lastname = 'Bar';

console.log(person.firstname, person.lastname); _// Foo Bar_
...
```
In this case, the message is logged once _all the contents of the page including the images and stylesheets(if any)_ are
...
```
In the above code, the message is logged only after the DOM/document is loaded ( _ie:once the DOM is constructed_ ).
...
Unlike in languages like Python, static properties of the constructor function are _not_ inherited to instances.
...
Callbacks offer a way to extend the functionality of a function (or method) **_without changing_** its code. This approach
...
makes sure that the code is called _after_ the JSON is fetched.
...
```
The following would _not_ work, because the data-handling code would likely be called _before_ the data is actually
...
```
y
stick y : matches only from the index indicated by the lastIndex property of this regular expression in the
...
match[ 1 ] is the text corresponding to the first captured group. match[n] would be the value of the _n_ th captured
...
JavaScript supports several types of group in its Regular Expressions, _capture groups_ , _non-capture groups_ and _look-_

_aheads_. Currently, there is no _look-behind_ support.

**Capture**

Sometimes the desired match relies on its context. This means a simple _RegExp_ will over-find the piece of the _String_
...
```
Say there is a _String_ where all + signs need to be replaced with a space, but only if they follow a letter character. This
...
```
Browser Google ChromeAndroid BrowserFirefoxiOS Safari
...
```
Browser Google ChromeOperaFirefoxSafariInternet Explorer
...
data persists between pages _in that window/tab_ for as long as it's open, but is visible nowhere else.
...
JSON stands for "JavaScript Object Notation", but it's not JavaScript. Think of it as just a _data serialization format_ that

_happens_ to be directly usable as a JavaScript literal. However, it is not advisable to directly run (i.e. through eval())
...
More confusion starts arising when _incorrect terminology_ is applied in code or in conversation.
...
2. replacer A function that alters the behaviour of the stringification process or an array of String and Number
...
_// Do not allow the object to be changed_
Object.freeze(enumObj);
**return** enumObj;
}();

console.log(testEnum.One); _// 1 will be logged_
...
**case** testEnum.Two:
console.log("222"); _// 222 will be logged_
**break** ;
}
...
The unary plus (+) precedes its operand _and evaluates_ to its operand. It attempts to convert the operand to a
...
**void** will always yield the _true_ **undefined** value though.
...
A _generator function_ is created with a **function** * declaration. When it is called, its body is **not** immediately executed.

Instead, it returns a _generator object_ , which can be used to "step through" the function's execution.
...
A generator is _iterable_. It can be looped over with a **for** ...of statement, and used in other constructs which depend
...
A Promise object represents an operation which _has produced or will eventually produce_ a value. Promises provide a
...
```
A promise is said to be _settled_ (or _resolved_ ) when it is either fulfilled or rejected. Once a promise is settled, it
...
. **catch** (console.error.bind(console)); _// outputs "oh yes"_
...
. **catch** (reason => console.log(reason)); _// outputs "Error!" after 6 seconds._
...
```
. **catch** (error => console.log(error.message)); _// outputs "bad things!" after 1 second_
...
```
you can promisify it _(convert it to a promise-based function)_ like this:
...
throwErrorAsync()
```
. **catch** (error => { _/* handle error here */_ });
...
.then(() => { /* never called */ })
```
. **catch** (error => { _/* handle error from throwErrorSync() */_ });
...
throwErrorAsync()
```
. **catch** (error => { _/* handle error here */_ ; **return** result; })
.then(result => { _/* handle result here */_ });
...
```
. **catch** (error => {
_/* handle error from throwErrorAsync() */_
**throw** error;
})
.then(() => { _/* will not be called if there's an error */_ })
. **catch** (error => { _/* will get called with the same error */_ });
...
.then(() => { /* will not be called */ })
```
. **catch** (error => { _/* handle error*/_ });
_// or_
throwErrorAsync()
.then(() => { _/* will not be called */_ }, error => { _/* handle error*/_ });
...
```
. **catch** (err => console.log(err)) _// <-- Error: UnexpectedValue will be caught here_
...
```
. **catch** (err => console.log(err)) _// <-- will not catch_
...
```
. **catch** ( **function** (e) {
_// Invalidate the current result to retry on the next fetch_
cachedPromise = **null** ;
_// re-raise the error to propagate it to callers_
**throw** e;
});
}
...
_SameValueZero_ comparison algorithm.
...
Show a modal _popup_ with a message to the user.
...
Show a modal _popup_ with the provided message.
...
Show a modal _popup_ with the provided message and an input field with an optional pre-filled value.
...
When using **prompt** a user can always click **_Cancel_** and no value will be returned.
...
A way to use confirm() is when some UI action does some _destructive_ changes to the page and is better
...
The alert() method of the window object displays an _alert box_ with a specified message and an OK or Cancel
...
An _alert box_ is often used if you want to make sure information comes through to the user.
...
**Edge**. If you need _better_ support for other browsers and more features like HTMLTable editing etc. a rule of thumbs
...
common keyboard shortcuts for formatting ( Ctrl-B for **bold** , Ctrl-I for _italic_ , etc.) or by dragging and
...
document.execCommand('underline', **false** , **null** ); _// toggles underline_
...
objects that model things like _history, device's screen,_ etc
...
The vast majority of modern JavaScript environments work according to an _event loop_. This is a common concept in

computer programming which essentially means that your program continually waits for new things to happen,

and when they do, reacts to them. The _host environment_ calls into your program, spawning a "turn" or "tick" or

"task" in the event loop, which then _runs to completion_. When that turn has finished, the host environment waits for
...
How this works is that when these statements execute, they tell the _host environment_ (i.e., the browser or Node.js
...
arguments object behave different in _strict_ and _non strict_ mode. In _non-strict_ mode, the argument object will reflect

the changes in the value of the parameters which are present, however in _strict_ mode any changes to the value of
...
_strict_ mode, the same will not be reflected.
...
both _strict-mode_ or _non-strict_ mode the arguments object remains unchanged.
...
```
var blob = new Blob([" \x 01 \x 02 \x 03 \x 04"]),
...
template literal but _exactly_ as they appear in the source code, without any backslash-escapes being replaced.
...
```
```
strings: ["Hello ", ". \n\n How are you?", raw: ["Hello ", ". \\ n \\ nHow are you?"]]
substitutions: ["world"]
```
...
When a function is declared, variables in the context of its _declaration_ are captured in its scope. For example, in the
...
_Immediately-invoked function expressions_ (or, perhaps more intuitively, _self-executing anonymous functions_ ) are
...
_(Note: All examples using_ **_let_** _are also valid for_ **_const_** _)_
...
```
Note that a _default export_ is implicitly equivalent to a named export with the name **default** , and the imported
...
**Note:** The path to the module './test' is resolved by the _loader_ and is not covered by the ECMAScript specification
...
whether it is _truthy_ or _falsy_.
...
!![] // true
```
The first negation converts any value to **false** if it is _truthy_ and to **true** if is _falsy_. The second negation then operates

on a normal Boolean value. Together they convert any _truthy_ value to **true** and any _falsy_ value to **false**.
...
[http://stackoverflow.com/documentation/javascript/208/boolean-logic/3047/double-negation-x](http://stackoverflow.com/documentation/javascript/208/boolean-logic/3047/double-negation-x)
...
```
Values that get converted to **false** as booleans are called _falsy_ (and all others are called _truthy_ ). See Comparison
...
```
Value Converted To StringConverted To NumberConverted To Boolean
...
as a _lambda expression_ due to Common Lisp similarities.
...
const bar = (baz) => baz + 1 ;
```
However, if the arrow function takes no parameters, or more than one parameter, a new set of parentheses _must_
...
By using the arrow function, I _preserve_ the **this** context so that I can make reference to **this** .sound when it comes
...
worker.terminate();
```
_Note_ : The terminate method is not available for service workers. It will be terminated when not in use, and
...
_'Prefer composition over inheritance'_ is an important and popular programming principle, used to assign behaviors to
...
characteristics is that they are _unique_ , even if they have the same description. This means they will never have a
...
let a = Symbol. for ('A');
```
The next time you call Symbol. **for** ('A'), the _same symbol_ will be returned instead of a new one (in contrast to
...
```
4. Create a scripts folder to store your .js files, and then a dist/scripts folder where the transpiled fully
...
```
6. Edit your package.json file (created when you ran npm init) and add the build script to the scripts
    property:
...
1. When, as the program is parsed from left to right, a token (called the _offending token_ ) is
...
3. When, as the program is parsed from left to right, a token is encountered that is allowed by some
    production of the grammar, but the production is a _restricted production_ and the token would be
...
Proxy on an object is itself an object, that has _traps_. Traps may be triggered when operations are performed on the
...
_// you could also access the object via proxied.target_
...
window. The parent/sender page will be assumed to be [http:](http:) _//sender.com_ and child/receiver page will be

assumed to be [http:](http:) _//receiver.com_ for the example.
...
For this example, since our receiver is assumed to be [http:](http:) _//receiver.com_ page, we enter its url as the
...
The code under this part should be put in the receiver page, which is [http:](http:) _//receiver.com_ for our example.
...
_// b reference to null_
> b = **null** ;
**null**

_// garbage collection_
> global.gc();
**undefined**

_// after remove b reference to object_ ， _heapUsed is 4M again
// it means the big array in WeakMap is released
// it also means weekmap does not contribute to big array's reference count, only b does._
> process.memoryUsage();
{ rss: 20639744 ,
heapTotal: 8425472 ,
heapUsed: 3979792 ,
...
_escape sequences_. Escape sequences consist of a backslash ("\\") followed by one or more other characters. To write
...
topic. For instance, the following escape sequences all denote the same character: the _line feed_ (Unix newline
...
```
var str = "ONE \x a0LINE"; // ONE and LINE with a non-breaking space between them
```
...
note that \0, unlike \00 or \000, is _not_ considered an octal escape sequence, and is thus still allowed inside strings
...
The Observer pattern is used for event handling and delegation. A _subject_ maintains a collection of _observers._ The
...
```
This will loop over the list of IDs to create an array of promises. await will wait for _all_ promises to be complete.
...
A JavaScript AsyncIterator is an object with a .next() method, which returns a Promise<IteratorItem>, a _promise_

for the next value.

To create an AsyncIterator, we can use the _async generator_ syntax:
...
image _/*_ for allowing all types of images) give you a quick and powerful way to restrict the type of files you want to
...
Whenever a user searches on that website, they are redirected to https: _//example.com/search?q=brown+puppies_.
...
```
That adds the message I like pie, but it also **downloads and runs https:** **_//alice.evil/js_xss.js_** **whenever**
...
2. Instead of dynamically building a script, build a plain text file containing all the messages that is later fetched
    by the script
3. **Add a Content Security Policy that refuses to load active content from other domains**
...
The **origin** of two addresses is considered the same if both URLs have the same _protocol_ , _hostname_ and _port_.
...
**JSON** with **P** adding is commonly blamed to be a workaround. It is not the most straightforward method, but it still
...
```
The contents of [http:](http:) _//example.com/api/endpoint.js?callback=myfunc_ might look like this:
...
_// window.DEBUG_ENVIRONMENT a configurable property that may be set to true somewhere else for
debugging and testing purposes._
...
3. Use the folder icon near the upper-left corner of the window to open a file-selection pane; you can find your
...
Clicking the _Select an element in the page to inspect it_ button in the upper left corner of the Elements tab in
...
```
However, you can refactor to move the slow code into a separate function (that _can_ be optimized) and call it from
...
performance-heavy even in the simplest of cases. The process of re-drawing a page is known as _reflow_ and can
...
consider a **memoizer** which is a _cache of previous function executions and their returned values_. This allows you to
...
Now that I start to reflect on what I did, if I were to increment from 1 instead of decrement from _num_ , I could have

cached all of the factorials from 1 to _num_ in the cache recursively, but I will leave that for you.
...
Certain words - so-called _keywords_ - are treated specially in JavaScript. There's a plethora of different kinds of

keywords, and they have changed in different versions of the language.

### Section A.1: Reserved Keywords

**JavaScript has a predefined collection of** **_reserved keywords_** **which you cannot use as variables, labels, or**
...
There was no change since _ECMAScript 3_.

_ECMAScript 5_ removed int, byte, char, **goto** , long, final, float, short, double, native, throws, boolean, abstract,
...
With regards to reserved words there is a small distinctions between the _"Identifiers"_ used for the likes of variable or

function names and the _"Identifier Names"_ allowed as properties of composite data types.
...
That conclusively tells us that, _by specification_ , it is allowed to have ReservedWords such as class and **var** as
...
**_Note:_** the syntax highlighter in this example has spotted the reserved word and still highlighted it. While the

example is valid JavaScript developers can get caught out by some compiler / transpiler, linter and minifier tools that argue otherwise.